...
export class NotificationsService {
  ...
  inbox(organizationId: string, userId: string, opts: { unreadOnly?: boolean; tab?: string } = {}) {
    ...
    return this.db.select().from(schema.notifications).where(and(...conds)).orderBy(desc(schema.notifications.createdAt)).limit(250);
  }
