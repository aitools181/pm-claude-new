...
export class UxController {
  ...
  @Patch("me/profile") updateProfile(@Req() r: Ctx, @Body(new ZodPipe(z.object({ displayName: z.string().trim().min(1).max(200).optional() }))) b: { displayName?: string }) { return this.ux.updateProfile(r.organizationId, r.userId, b); }
  @Get("workspace/settings") workspaceSettings(@Req() r: Ctx) { return this.ux.workspaceSettings(r.organizationId); }
  @Patch("workspace/settings") @RequirePermission(CAPABILITIES.ORG_SETTINGS_MANAGE) updateWorkspaceSettings(@Req() r: Ctx, @Body(new ZodPipe(z.object({ name: z.string().trim().min(1).max(200).optional(), timezone: z.string().min(1).max(100).optional(), weekStart: z.number().int().min(0).max(6).optional(), dateFormat: z.string().max(40).optional(), branding: z.record(z.unknown()).optional() }))) b: any) { return this.ux.updateWorkspaceSettings(r.organizationId, r.userId, b); }
