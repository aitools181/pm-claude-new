...
export function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <aside className="work-sidebar" data-open={open} data-compact-sections={!!nav.compactSections} aria-label="Work navigation">
    ...
    <div className="work-sidebar-bottom">
      ...
      <button type="button" className="sidebar-invite-btn" onClick={() => window.dispatchEvent(new CustomEvent("pm:open-create", { detail: { kind: "invite" } }))}><Icon name="user" size={16} />Quick invite</button>
      {isPlatformAdmin && <a href="/superadmin"><Icon name="shield" size={16}/>Platform console</a>}<a href="/admin/organizations"><Icon name="settings" size={16}/>Organizations</a><a href="/admin/configure"><Icon name="sliders" size={16}/>Customize</a><a href="/settings/workspace#plan"><Icon name="star" size={16}/>Plan & billing</a><a href="/settings/workspace"><Icon name="settings" size={16}/>Workspace settings</a><a href="/admin/people"><Icon name="people" size={16}/>Manage people</a><button type="button" className="sidebar-signout" data-testid="sidebar-sign-out" onClick={() => { void signOut(); }}><Icon name="arrowLeft" size={16}/>Log out</button></div>
  </aside>;
}
