...
export default function GanttPage() {
  ...
  async function captureBaseline() {
    const name = prompt("Baseline name", `Baseline ${new Date().toLocaleDateString()}`); if (!name) return;
  ...
  return (
    <>
      ...
      <div className="view-toolbar project-toolbar"><button className="toolbar-button">Today</button><button className="toolbar-button"><Icon name="filter" size={15}/>Filter</button><button className="toolbar-button"><Icon name="sort" size={15}/>Sort</button><button className="toolbar-button"><Icon name="sliders" size={15}/>Options</button><select className="toolbar-select" value={baselineId} onChange={(e) => setBaselineId(e.target.value)}><option value="">No baseline</option>{baselines.map((base)=><option key={base.id} value={base.id}>{base.name}</option>)}</select><button className="toolbar-button" onClick={captureBaseline}>Capture baseline</button><button className="toolbar-button" onClick={() => setPxPerDay((p) => Math.max(6, p - 4))}>−</button><button className="toolbar-button" onClick={() => setPxPerDay((p) => Math.min(56, p + 4))}>+</button></div>

      {lastOp && <div className="undo-banner"><span>Rescheduled {lastOp.applied} item(s).</span><button className="btn btn-ghost" onClick={undo}>Undo</button></div>}
      <div className="legend">
        <span><span className="sw" style={{ background: "var(--danger)" }} />Critical path</span>
        <span><span className="sw" style={{ background: "var(--primary)" }} />On schedule</span>
        <span><span className="sw" style={{ background: "var(--primary-weak)" }} />Slack</span>
        {baselineId && <span><span className="sw" style={{ border: "1px dashed var(--ink-3)", background: "transparent" }} />Baseline</span>}
      </div>

      <div className="gantt-layout">
        <div className="gantt-wrap">
          {!model && <div className="empty" style={{ padding: 24 }}>No scheduled items yet. Set start/due dates or durations.</div>}
          {model && (
            <svg width={model.width} height={model.height} style={{ display: "block" }}>
        ...
        <div className="gpanel">
          ...
          {sel && (
            <>
              <h3><span className="mono" style={{ color: "var(--ink-3)", fontSize: 12 }}>{sel.key}</span> {sel.title}</h3>
              <p className="muted">{sel.critical ? "On the critical path" : `Slack: ${sel.slack} working day(s)`}</p>

              <div style={{ margin: "14px 0" }}>
                <div className="muted" style={{ marginBottom: 5 }}>Schedule mode</div>
                <div className="seg">
              ...
              <div style={{ margin: "14px 0" }}>
                <div className="muted" style={{ marginBottom: 5 }}>Duration (working days)</div>
                <input className="input" type="number" min={1} defaultValue={selMeta?.durationDays ?? ""} placeholder="from dates"
                  onBlur={(e) => e.target.value !== String(selMeta?.durationDays ?? "") && setDuration(e.target.value)} style={{ width: 120 }} />
              </div>

              <div style={{ borderTop: "1px solid var(--line)", margin: "14px 0", paddingTop: 14 }}>
                <div className="muted" style={{ marginBottom: 5 }}>Reschedule start to</div>
                <div style={{ display: "flex", gap: 8 }}>
                  <input className="input" type="date" value={moveTo} onChange={(e) => setMoveTo(e.target.value)} />
                  <button className="btn btn-primary" disabled={!moveTo} onClick={runPreview}>Preview</button>
                </div>
                <p className="muted" style={{ marginTop: 6 }}>Preview shows the cascade before anything is saved.</p>
              </div>
            </>
          )}
        </div>
      ...
      {preview && (
        <div className="modal-backdrop" onClick={() => setPreview(null)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            <h2 className="page-title" style={{ fontSize: 18 }}>Reschedule impact</h2>
            ...
            <div style={{ margin: "10px 0" }}>
              ...
              {preview.changes.map((c) => (
                <div key={c.itemId} className={`diff-row ${c.manualConflict ? "conflict" : ""}`}>
                  ...
                  <span>{c.title}{c.manualConflict && <span className="badge" style={{ marginLeft: 6, background: "var(--danger-weak)", color: "#8f2b27" }}>manual conflict</span>}</span>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 12 }}>
              ...
              <button className="btn btn-primary" onClick={confirmReschedule} disabled={preview.changedCount === 0}>Confirm & apply</button>
            </div>
          </div>
        </div>
      )}
