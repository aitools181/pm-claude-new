...
export default function WhiteboardPage() {
  ...
  async function newBoard() { const name = await appPrompt("Board name"); if (!name) return; try { const b = await api<Board>("/whiteboards", { method: "POST", org: true, body: JSON.stringify({ name }) }); loadBoards(); open(b.id); } catch (e) { toast({ message: e instanceof ApiError ? e.message : "Could not create the board" }); } }
  async function addNote() { if (!sel) return; const label = await appPrompt("Note text"); if (!label) return; try { await api(`/whiteboards/${sel}/elements`, { method: "POST", org: true, body: JSON.stringify({ kind: "note", x: 40 + Math.random() * 300, y: 40 + Math.random() * 300, data: { label } }) }); open(sel); } catch (e) { toast({ message: e instanceof ApiError ? e.message : "Could not add the note" }); } }
  async function toTask(e: El) { if (!proj) { toast({ message: "Pick a project" }); return; } try { await api(`/whiteboards/elements/${e.id}/to-task`, { method: "POST", org: true, body: JSON.stringify({ projectId: proj }) }); toast({ message: "Work item created from element" }); open(sel!); } catch (err) { toast({ message: err instanceof ApiError ? err.message : "Could not create a task from this element" }); } }
