...
function AccountSettingsInner(){
  ...
  async function load(){const [a,b,c,d]=await Promise.all([api<Two>('/2fa/status').catch(()=>({enabled:false})),api<EmailRow[]>('/me/emails',{org:true}).catch(()=>[]),api<OrgRow[]>('/me/workspaces',{org:true}).catch(()=>[]),api<Workspace[]>('/workspaces',{org:true}).catch(()=>[])]);setTwo(a);setEmails(b);setOrgs(c);setWorkspaces(d)}
