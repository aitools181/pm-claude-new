...
type Two={enabled:boolean};
type EmailRow={id:string;email:string;label?:string|null;verifiedAt?:string|null;primary:boolean};
...
function AccountSettingsInner(){
  ...
  const [mergeEmail,setMergeEmail]=useState(''); const [mergePassword,setMergePassword]=useState(''); const [merging,setMerging]=useState(false);
  const [orgs,setOrgs]=useState<OrgRow[]>([]); const [workspaces,setWorkspaces]=useState<Workspace[]>([]); const [workspaceName,setWorkspaceName]=useState('');
  const [loadError,setLoadError]=useState('');
  async function load(){try{const [a,b,c,d]=await Promise.all([api<Two>('/2fa/status'),api<EmailRow[]>('/me/emails',{org:true}),api<OrgRow[]>('/me/workspaces',{org:true}),api<Workspace[]>('/workspaces',{org:true})]);setTwo(a);setEmails(b);setOrgs(c);setWorkspaces(d);setLoadError('')}catch(e){setLoadError(e instanceof ApiError?e.message:'Could not load account settings.')}}
  useEffect(()=>{load()},[]);
  useEffect(()=>{const token=qs.get('verifySecondary');if(!token)return;api<{email:string}>('/me/emails/verify',{method:'POST',org:true,body:JSON.stringify({token})}).then(r=>{toast({message:`${r.email} verified`,tone:'success'});load();history.replaceState({},'',location.pathname)}).catch(e=>toast({message:e instanceof ApiError?e.message:'Could not verify email',tone:'error'}))},[qs]);
  async function change(){try{await api('/auth/password-change',{method:'POST',body:JSON.stringify({currentPassword:current,newPassword:next})});location.assign('/login')}catch(e){setMsg(e instanceof ApiError?e.message:'Could not change password')}}
  async function addEmail(){if(!newEmail.trim())return;try{await api('/me/emails',{method:'POST',org:true,body:JSON.stringify({email:newEmail.trim(),label:newLabel.trim()||undefined})});setNewEmail('');setNewLabel('');toast({message:'Verification email sent',tone:'success'});load()}catch(e){toast({message:e instanceof ApiError?e.message:'Could not add email',tone:'error'})}}
  async function makePrimary(id:string){await api(`/me/emails/${id}/make-primary`,{method:'POST',org:true});toast({message:'Primary email updated',tone:'success'});load()}
  ...
  return <SettingsShell><div className="settings-section"><h2>Account</h2><p>Password, identity emails, organizations/workspaces, two-factor authentication, and sessions.</p>
    ...
    {loadError && <div className="callout callout-danger account-load-error"><span>{loadError}</span><Button variant="secondary" size="compact" onClick={load}>Retry</Button></div>}
    <div className="setting-card"><strong>Two-factor authentication</strong><span>{two?(two.enabled?'Enabled':'Not enabled'):loadError?'Could not check':'Checking…'}</span><a className="btn" href="/settings/sessions">Manage security & sessions</a></div>
    <div className="setting-card"><strong>Sign out</strong><span className="setting-copy">End the session on this device and return to the sign-in screen.</span><Button variant="secondary" data-testid="account-sign-out" onClick={()=>{void signOut()}}>Log out</Button></div>
