  ...
  return (
    <>
      ...
      <div className="card" style={{ marginBottom: 18 }}>
        <div className="card-head"><div><h2 style={{ margin: 0, fontSize: 16 }}>Active sessions</h2><p className="muted" style={{ margin: "4px 0 0" }}>Revoke any device you no longer recognise.</p></div>
          <button className="btn btn-secondary" disabled={busy || !rows?.length} onClick={() => action(async () => {
            await api("/auth/sessions/revoke-all", { method: "POST" }); window.location.href = "/login";
          }, "All sessions revoked")}>Sign out everywhere</button></div>
        <div className="table-wrap"><table className="table">
          ...
          <tbody>
            ...
            {rows?.map((s) => <tr key={s.id}>
              <td><strong>{s.current ? "This device" : "Signed-in device"}</strong><div className="muted" style={{ maxWidth: 420, fontSize: 12 }}>{s.userAgent ?? "Unknown device"}</div></td>
              <td className="mono">{s.ip ?? "—"}</td><td>{new Date(s.createdAt).toLocaleString()}</td><td>{new Date(s.expiresAt).toLocaleDateString()}</td>
              <td><button className="btn btn-ghost btn-sm" disabled={busy} onClick={() => action(async () => {
                const result = await api<{ current: boolean }>(`/auth/sessions/${s.id}`, { method: "DELETE" });
                if (result.current) window.location.href = "/login";
              }, "Session revoked")}>Revoke</button></td>
            </tr>)}
          </tbody>
        </table></div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 18 }}>
        <div className="card-head"><div><h2 style={{ margin: 0, fontSize: 16 }}>Email verification</h2><p className="muted" style={{ margin: "4px 0 0" }}>Send a single-use verification link to your account email.</p></div>
          <button className="btn btn-secondary" disabled={busy} onClick={() => action(() => api("/auth/email-verification/request", { method: "POST" }), "Verification email queued")}>Send verification email</button></div>
      </div>

      <div className="card">
        <div className="card-head"><div><h2 style={{ margin: 0, fontSize: 16 }}>Two-factor authentication</h2><p className="muted" style={{ margin: "4px 0 0" }}>{twofa?.enabled ? `Enabled · ${twofa.recoveryCodesRemaining} recovery codes remaining` : "Add an authenticator app for stronger account protection."}</p></div>
          {!twofa?.enabled && !enrolment && <button className="btn btn-primary" disabled={busy} onClick={begin2fa}>Set up 2FA</button>}</div>

        {enrolment && <div className="settings-split">
          <div><img src={enrolment.qrDataUrl} width={220} height={220} alt="Authenticator QR code" style={{ border: "1px solid var(--line)", borderRadius: 12 }} /></div>
          <div><Callout tone="info">Scan the QR code, or enter this secret manually: <span className="mono">{enrolment.secret}</span></Callout>
            ...
            <button className="btn btn-primary" disabled={busy || code.length < 6} onClick={confirm2fa}>Confirm and enable</button></div>
        </div>}

        {twofa?.enabled && <div style={{ maxWidth: 520 }}>
          ...
          <div className="button-row"><button className="btn btn-secondary" disabled={busy || code.length < 6} onClick={regenerate}>Generate new recovery codes</button>
            <button className="btn btn-danger" disabled={busy || code.length < 6} onClick={disable2fa}>Disable 2FA</button></div>
        </div>}

        {recoveryCodes.length > 0 && <div style={{ marginTop: 18 }}><Callout tone="danger"><strong>Save these recovery codes now.</strong> Each code works once and will not be shown again.</Callout>
