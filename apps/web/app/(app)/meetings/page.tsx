...
export default function MeetingsPage() {
  ...
  async function createMeeting() { const t = await appPrompt("Meeting title"); if (!t) return; try { const m = await api<Meeting>("/meetings", { method: "POST", org: true, body: JSON.stringify({ title: t, scheduledAt: new Date().toISOString() }) }); await load(); open(m.id); } catch (e) { toast({ message: e instanceof ApiError ? e.message : "Could not create the meeting" }); } }
  async function saveNotes() { if (!sel) return; try { await api(`/meetings/${sel}/notes`, { method: "PUT", org: true, body: JSON.stringify({ notes }) }); toast({ message: "Notes saved" }); } catch (e) { toast({ message: e instanceof ApiError ? e.message : "Could not save notes" }); } }
  async function saveTranscript() { if (!sel) return; try { await api(`/meetings/${sel}/transcript`, { method: "POST", org: true, body: JSON.stringify({ transcript }) }); toast({ message: "Transcript saved" }); } catch (e) { toast({ message: e instanceof ApiError ? e.message : "Could not save the transcript" }); } }
  async function extract() { if (!sel) return; try { const r = await api<{ candidates: string[] }>(`/meetings/${sel}/extract-actions`, { method: "POST", org: true, body: JSON.stringify({}) }); setCandidates(r.candidates); if (!r.candidates.length) toast({ message: "No action-like lines found in the transcript" }); } catch (e) { toast({ message: e instanceof ApiError ? e.message : "Could not extract actions from the transcript" }); } }
  async function candidateToAction(title: string) { if (!sel) return; try { await api(`/meetings/${sel}/actions`, { method: "POST", org: true, body: JSON.stringify({ title }) }); setCandidates((c) => c ? c.filter((x) => x !== title) : c); toast({ message: "Added as meeting action" }); open(sel); } catch (e) { toast({ message: e instanceof ApiError ? e.message : "Could not add this as a meeting action" }); } }
  async function addAgenda() { if (!sel) return; const t = await appPrompt("Agenda item"); if (!t) return; try { await api(`/meetings/${sel}/agenda`, { method: "POST", org: true, body: JSON.stringify({ title: t, position: (d?.agenda.length ?? 0) + 1 }) }); open(sel); } catch (e) { toast({ message: e instanceof ApiError ? e.message : "Could not add the agenda item" }); } }
  async function addDecision() { if (!sel) return; const t = await appPrompt("Decision"); if (!t) return; try { await api(`/meetings/${sel}/decisions`, { method: "POST", org: true, body: JSON.stringify({ text: t }) }); open(sel); } catch (e) { toast({ message: e instanceof ApiError ? e.message : "Could not record the decision" }); } }
  async function setAtt(userId: string, status: string) { if (!sel) return; try { await api(`/meetings/${sel}/attendance`, { method: "POST", org: true, body: JSON.stringify({ userId, status }) }); open(sel); } catch (e) { toast({ message: e instanceof ApiError ? e.message : "Could not update attendance" }); } }
